...
from .ddns_service import update_ddns
import time
import os
import logging
...
def run_backtest_task(job_id, timeframe_days: int = 365, symbol_override: str = None, initial_capital: float = 10000, selected_indicators: List[str] = None):
    ...
    try:
        ...
        data, provider_used = data_manager.get_data_from_any_provider(symbol, start_date, end_date)
        ...
        try:
            if 'close' in data.columns and data['close'].std(skipna=True) == 0:
                ...
                if provider_used != 'mt5':
                    alt_data, alt_provider = data_manager.get_data_from_any_provider(symbol, start_date, end_date)
        ...
        try:
            ...
            try:
                ...
                ai_analysis = analyze_backtest_trades_with_ai(
            ...
            # Combine original description with AI analysis
            original_description = result_data.get('description_fa', result_data.get('error', ''))
            if ai_analysis:
...
@shared_task
def update_ddns_task():
    """
    Periodic task to update DDNS configurations
    """
    try:
        logger.info("Running DDNS update task...")
        update_ddns()
        logger.info("DDNS update task completed")
    except Exception as e:
        logger.error(f"Error in DDNS update task: {e}")
