...
class BacktestEngine:
    ...
    def _parse_custom_strategy(self, data: pd.DataFrame, strategy: Dict[str, Any]) -> Tuple[pd.Series, Dict[int, Dict[str, str]]]:
        ...
        try:
            ...
            for condition in entry_conditions:
                ...
                for condition_part in condition_parts:
                    ...
                    # Parse generic BUY/SELL patterns (before specific indicators)
                            ...
                            if 'rsi' in data.columns:
                                ...
                                if signal_count > 0:
                                    ...
                                    if signal_count > 0 and signal_count < len(data) * 0.3:  # Not too many signals
                                        ...
                                        parsed_entry_conditions += 1
                                        condition_parsed = True
                            # If no RSI, check for volume-based conditions
                            elif 'volume' in data.columns:
                                # High volume buy signal
                                if 'پرحجم' in condition_lower or 'high volume' in condition_lower or 'volume' in condition_lower:
            ...
            if signals.sum() == 0:
                ...
                if signals.sum() == 0:
                    detailed_logger = logging.getLogger('ai_module.backtest_engine')
                    detailed_logger.error("=" * 80)
                    detailed_logger.error("===== STRATEGY PARSING DIAGNOSTICS ======")
                    detailed_logger.error(f"Entry conditions provided: {len(entry_conditions)}")
                    detailed_logger.error(f"Exit conditions provided: {len(exit_conditions)}")
                    detailed_logger.error(f"Parsed successfully: {parsed_entry_conditions} entry, {parsed_exit_conditions} exit")
                    detailed_logger.error(f"Indicators in strategy: {indicators}")
                    detailed_logger.error(f"Available data columns: {available_cols[:20]}...")  # First 20 columns
                    
                    # Show raw excerpt to help diagnose
                    if raw_excerpt:
                        detailed_logger.error(f"Raw excerpt (first 1000 chars):")
                        detailed_logger.error("-" * 80)
                        detailed_logger.error(raw_excerpt[:1000])
                        detailed_logger.error("-" * 80)
                    else:
                        detailed_logger.error("No raw excerpt available!")
                    
                    # Show actual entry/exit conditions that were provided
                    if entry_conditions:
                        detailed_logger.error(f"Entry conditions (first 3):")
                        for idx, cond in enumerate(entry_conditions[:3], 1):
                            detailed_logger.error(f"  {idx}. {cond[:200]}...")
                    else:
                        detailed_logger.error("⚠️ NO ENTRY CONDITIONS PROVIDED!")
                    
                    if exit_conditions:
                        detailed_logger.error(f"Exit conditions (first 3):")
        ...
        total_signals = int((signals == 1).sum()) + int((signals == -1).sum())
        logger.info(f"Final signal count: {total_signals} total ({(signals==1).sum()} buy, {(signals==-1).sum()} sell)")
