...
export default function APIUsageStats() {
  ...
  }, [isAdmin, selectedDays, selectedProvider])
  ...
  const loadStats = async () => {
    try {
      ...
      const response = await client.get<APIUsageStats>('/api-usage-stats/', { params })
      setStats(response.data)
    } catch (error: any) {
      console.error('Error loading API usage stats:', error)
      showToast('خطا در بارگذاری آمار مصرف API', { type: 'error' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          <p className="text-gray-400">خطا در بارگذاری آمار</p>
        </div>
      )}
    </div>
  )
