...
# Try to build first (ignore TypeScript errors for now)
Write-Host "Building Frontend (this may take a moment)..." -ForegroundColor Yellow
$buildResult = npm run build 2>&1
if ($LASTEXITCODE -ne 0) {
    Write-Host "Build had warnings, but continuing..." -ForegroundColor Yellow
}

# Start preview server on port 80
Write-Host ""
Write-Host "Starting preview server on port 80..." -ForegroundColor Green
