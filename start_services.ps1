...
if (-not $port3000.TcpTestSucceeded) {
    ...
    # Start Frontend in background (Development Mode)
    $frontendScript = @"
`$env:VITE_FRONTEND_PORT = "3000"
`$env:VITE_BACKEND_URL = "http://127.0.0.1:8000"
Set-Location "$PWD\frontend"
Start-Process powershell -ArgumentList "-NoExit", "-Command", "npm run dev -- --port 3000 --host 0.0.0.0"
"@
    ...
    Write-Host "   Frontend dev server starting in background..." -ForegroundColor Green
    ...
    if ($port3000Check.TcpTestSucceeded) {
        ...
        Write-Host "   Run manually: cd frontend && npm run dev -- --port 3000 --host 0.0.0.0" -ForegroundColor Cyan
    }
} else {
