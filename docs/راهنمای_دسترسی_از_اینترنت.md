...
- پورت‌های Backend و Frontend را تنظیم می‌کند (در صورت وارد کردن IP عمومی، پورت Frontend به صورت پیش‌فرض روی 80 قرار می‌گیرد تا از اینترنت بدون نیاز به پورت سفارشی قابل دسترس باشد)
...
4. Port -> TCP -> Specific local ports: `8000,80`
...
3. قوانین زیر را اضافه کنید:
   ...
   - **Frontend**: External Port `80` -> Internal IP سرور -> Internal Port `80`
...
# تست Frontend (در صورت تنظیم پورت 80)
Start-Process "http://localhost"
# اگر هنوز از پورت توسعه 3000 استفاده می‌کنید:
...
# تست Frontend
# مرورگر: http://YOUR_PUBLIC_IP
# در صورتی که پورت دیگری انتخاب کرده‌اید، آن را به انتهای آدرس اضافه کنید (مثلاً :8081)
```
...
1. **فایروال**: مطمئن شوید که پورت‌ها در فایروال باز هستند
   ...
   Get-NetFirewallRule -DisplayName "*Port 80*"
   ```
